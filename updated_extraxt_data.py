...
def extract_data(data):
    ...
    for rooms in data['roomTypes']:
        for d in rooms['offers']:
            ...
            rates.append({(data['id']+'_'+str(count)):rate_info})
        pprint(rates)
    json_data={
        "hotel_id":data['id'],
    ...
    df.to_csv(r"C:\Users\goura\Downloads\output.csv", index=False, mode='a')
...
for id in hotel_ids:
    url = f"https://www.qantas.com/hotels/api/ui/properties/{id}/availability?checkIn=2024-08-13&checkOut=2024-08-14&adults=2&children=0&infants=0&payWith=cash"
    response = requests.request("GET", url, headers=headers)
